...
* Branching workflow allows us to make changes to multiple files before fully committing to those changes.
...
* Save Work
* Outside of git, saving usually means the newer version *overwrites* the older version.
* Git solves this problem by providing you multiple save points.
* You can get the current version, and ANY previous version
* Git's philosophy: never lose anything.

### Commits

* Saving your work in git is known as committing
* Saving a files does not save it to git
...
* The repository on your computer is called the **local repository**
* It is only accessible through your computer
* When you push your repository to Github, you are creating a **remote repository**
* It is in the cloud and collaborators can access it through the web.
* In olden days, companies ran their own git servers (version of GitHub)
* GitHub has become the de facto community standard
* You certainly don't need GitHub to use git, but its popularity has intertwined the two
* don't forget that these are 2 distinct things
  * `git` provides the core technology for tracking
  * `GitHub` provides a shared location for hosting git repositories
